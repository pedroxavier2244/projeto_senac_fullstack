...
async function carregarDadosUsuario(id) {
    // Adicione aqui o código para carregar dados do usuário se necessário
    // Exemplo:
    // const token = sessionStorage.getItem('JWT_TOKEN');
    // fetch(`/usuarios/${id}`, {
    //     headers: {
    ...
    // })
    // .then(usuario => {
    //     // Lógica para manipular os dados do usuário
    // })
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (autorizado) {
        ...
        if (id) {
            carregarProduto(id);
            carregarDadosUsuario(id); // Se necessário, caso contrário, remova essa linha
        }
    }
});
...
document.getElementById('form-editar').addEventListener('submit', function (event) {
    ...
    .catch(error => {
        Swal.fire({
            ...
            location.reload(); // Recarrega a página para permitir nova tentativa
        });
    });
});
