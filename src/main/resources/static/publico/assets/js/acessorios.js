...
document.addEventListener("DOMContentLoaded", () => {
   ...
   document.getElementById("filtrar").addEventListener("click", () => {
       const minPrice1 = parseFloat(document.getElementById("minPrice").value) || 0;
       const maxPrice1 = parseFloat(document.getElementById("maxPrice").value) || Infinity;
...
async function fetchProdutos(pagina) {
    try {
        const response = await fetch(`http://localhost:8080/produtos/procurarCategoria?page=${pagina - 1}&size=${produtosPorPagina}&categoriaId=${3}`);
    ...
    try {
        if (ordenarPor === "") {
            Swal.fire({
                ...
                text: 'Você precisa selecionar ao menos uma opção para a busca.',
                icon: 'info',
        ...
        const response = await fetch(`http://localhost:8080/produtos/filtro/listaPorCategoriaEPreco?categoriaId=${3}&minPrice=${minPrice1}&maxPrice=${maxPrice1}`);

        if (!response.ok) {
            throw new Error('Não POde Obter resposta: ' + response.statusText);
        }
        ...
        Swal.fire({
            ...
            text: 'Voçê precisa definir um valor minino e maximo ',
            icon: 'error',
            confirmButtonText: 'Ok'
        });
    }
