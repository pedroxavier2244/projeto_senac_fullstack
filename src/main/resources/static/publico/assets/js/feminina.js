...
async function fetchProdutos(pagina) {
    try {
        const response = await fetch(`http://localhost:8080/produtos/procurarCategoria?page=${pagina - 1}&size=${produtosPorPagina}&categoriaId=${2}`);
...
async function fetchAllProdutos(minPrice1, maxPrice1) {
    ...
    try {
        ...
        const response = await fetch(`http://localhost:8080/produtos/filtro/listaPorCategoriaEPreco?categoriaId=${2}&minPrice=${minPrice1}&maxPrice=${maxPrice1}`);
