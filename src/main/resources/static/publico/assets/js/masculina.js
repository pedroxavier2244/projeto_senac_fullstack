...
async function fetchProdutos(pagina) {
  try {
    const response = await fetch(
      `http://localhost:8080/produtos/procurarCategoria?page=${
        pagina - 1
...
async function fetchAllProdutos(minPrice1, maxPrice1) {
  ...
  try {
    ...
    const response = await fetch(
      `http://localhost:8080/produtos/filtro/listaPorCategoriaEPreco?categoriaId=${1}&minPrice=${minPrice1}&maxPrice=${maxPrice1}`
    );
