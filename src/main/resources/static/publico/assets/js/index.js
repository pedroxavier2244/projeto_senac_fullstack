...
async function pesquisaProdutos(filtro = '') {
    try {
...
function exibirProdutos(produtos) {
    ...
    if (produtos.length > 0) {
        const produto = produtos[0]; // Pega apenas o primeiro produto
        const produtoElement = document.createElement('div');

        // Aqui você pode formatar o HTML como quiser
...
async function fetchProdutos(pagina) {
    try {
        ...
        exibirProdutos(produtos); // Exibe a lista de produtos por categoria
...
// Botões de comprar e adicionar interagem com o carrinho
